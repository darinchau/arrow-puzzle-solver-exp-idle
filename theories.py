...
def find_subsequence(arr, subarr, threshold=5, match=0.9):
    s = len(arr) - len(subarr) + 1
    rolling = np.zeros((s,) + subarr.shape)
    for i in range(s):
        rolling[i] = arr[-len(subarr):]
        arr = arr[:-1]
    n = [navigator.compare(a, subarr, threshold, match) for a in rolling]
    return np.flip(n)
...
def GetVariableValue(x1, y1, x2, y2, screen: np.ndarray, tolerance=5):
    ...
    multiple = cropAndFilter(
        x1, y1, x2, y1 - first_viable_pixel - len(check.valuee), screen)
...
def Publish():
    # Publish
    clickReverse(154, 38)
    clickReverse(710, 400, repeat=1, wait=True)
    clickReverse(785, 400, repeat=1, wait=True)
    clickReverse(880, 400, repeat=1, wait=True)
...
def T5():
    ...
    # Publish at multiplier >= 7.62
        ...
        BuyUntil(787, 135, 819, 1, 0.1)
        BuyUntil(611, 135, 643, 1, 0.1)
        setTimer(2 * rho)
...
def T6():
    ...
    if LastPublishMark == 0:
        if not navigator.IsPublishable(screen):
            ...
            nextpub = np.log10(PublishMultiplier) + rho
            LastPublishMark = rho
            print(f"Theory 6: Publishing. Next publish at: e{nextpub}")
            Publish()
        return

    if rho < LastPublishMark + np.log10(PublishMultiplier):
        # Set Checkboxes and buy c1
        print("Theory 6: Waiting")
        setTimer(2 * rho)
    else:
        nextpub = np.log10(PublishMultiplier) + rho
        print(f"Theory 6: Publishing. Next publish at: e{nextpub}")
